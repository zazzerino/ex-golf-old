defmodule Golf.Game do
  ...
  def handle_event(%{state: :uncover_two} = game, %{action: :uncover} = event) do
    ...
    if Player.uncovered_two?(game.players[player_id]) do
      {:ok, game}
    else
      players = Map.update!(game.players, player_id, &Player.uncover_card(&1, hand_index))
      all_uncovered_two? = Enum.all?(Map.values(players), &Player.uncovered_two?/1)
      state = if all_uncovered_two?, do: :take, else: game.state
  ...
  defp check_game_over(player_id, players, final_turn?) do
    state =
      if Enum.all?(Map.values(players), &Player.all_uncovered?/1) do
        :over
      else
        :take
      end
    ...
    final_turn? =
      if Player.all_uncovered?(players[player_id]) do
        true
      else
        final_turn?
      end
